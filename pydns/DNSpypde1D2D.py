...
class BWH(PDEBase):
    ...
    #definition of the function for IC
    #if a special IC is used it can be input through Bini, Wini, Hini during call
    #def get_initial_state(self, grid, storage, Bini, Wini, Hini):
    
    #if random IC is used then no need input of Bini, Wini, Hini
    def get_initial_state(self, grid, storage, Lx, mesh):
        # Random IC
        B = ScalarField.random_uniform(grid); W = ScalarField.random_uniform(grid)
        H = ScalarField.random_uniform(grid)

        #if IC is input during call, modify B, W, H from Bini, Wini, Hini respectively
        #one can also modify the random IC by accessing B, W, H data as below
        #B.data = Bini; W.data = Wini; H.data = Hini
        
        #example of changing IC
        x = np.linspace(0, Lx, mesh)
        B.data = 0.5 + (0.01 * np.sin(2 * np.pi * 4 * x/Lx))
        W.data = 1.2; H.data = 0.25

        #this labels are useful when output data file is analysed later
        B.label = "Plants"; W.label = "Water"; H.label = "Herbivore"
...
#reading from data file, the IC
#
#f = h5py.File('data.h5','r')
## This data file is not provided but it can generated in the way we have described in the beginning.
#keylist = list(f.keys())
#data = f[keylist[0]]
#Bini = data[1000][0]    #here the final time data (frame no: 10000)is read and used as IC to Bini and Wini
#Wini = data[1000][1]
...
for p in P_list:
    ...
    #generally if random IC is used, then no need for Bini, Wini or Hini as input
